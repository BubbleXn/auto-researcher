    ...
    async for event in graph.astream(input_or_cmd, config=config):
        ...
        for node_name, node_output in event.items():
            for sse_dict in node_output.get("_sse_events", []):
                collected.append(SSEEvent.model_validate(sse_dict))
    return collected
...
@pytest.mark.asyncio
async def test_full_graph_planner_to_writer(checkpointer) -> None:
...
@pytest.mark.asyncio
async def test_graph_retry_loop(checkpointer) -> None:
